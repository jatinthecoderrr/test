...
const Navbar = () => {
  ...
  const toggleSidebar = () => {
    console.log('Hamburger clicked. Sidebar open:', !isSidebarOpen);
  ...
  return (
    <header className={`w sticky top-0 pt-2 z-50 transition-all duration-300 ${scrolled ? "bg-white shadow translate-y-0" : "bg-transparent translate-y-9"}`}>
      {/* Upper Header */}
      ...
      <div className="flex justify-between items-center mt-2  md:mt-0 px-4 md:px-4 pb-3 md:mr-5 ">
        ...
        {/* Mobile menu button */}
<button
  className="lg:hidden z-[70] text-gray-800 hover:text-[#FC2411]" 
...
{/* Mobile Sidebar */}
<div
  className={`fixed top-0 left-0 h-screen w-4/5 max-w-sm bg-white z-[60] shadow-xl transform transition-transform duration-300 ease-in-out
  ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:hidden overflow-y-auto`}
>
  ...
  {/* Sidebar menu content (unchanged) */}
