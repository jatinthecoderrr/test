...
function FeatureSectionRev({
  ...
  return (
    <section
      className="w-full py-10 px-8 sm:py-14 md:py-16 md:px-18 bg-cover bg-center bg-no-repeat"
      style={{
    ...
    >
      <div className="max-w-[1500px] mx-auto lg:px-0 grid grid-cols-1 md:grid-cols-2 items-center gap-8 md:gap-12">
        ...
        >
          <div className="text-left space-y-2 sm:space-y-2 lg:space-y-4 sm:mr-10 ">
            {/* Small Title */}
            <h5
            ...
            {description1 && (
              <p
                className={`text-gray-700 text-sm sm:text-base md:text-[18px] md:pb-1 leading-relaxed transition-all duration-700 delay-300 ${
                  textVisible
                    ? "translate-x-0 opacity-100"
                    : "translate-x-10 opacity-0"
                }`}
          ...
          >
            {image && (
              <img
                src={image}
                alt={title}
                className="w-full h-90 md:h-[300px] object-contain rounded-3xl"
              />
            )}
          </div>
        </div>
...
export default FeatureSectionRev;


//how to use this component in a page
// import FeatureSectionRev from "";
